...
const Update = () => {
  ...
  console.log(goalId);
  const url = `https://goalapi-n0s2.onrender.com/api/goals/${goalId}`;
  const getGoal = async () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          "Congratulation Your Goal have been Updated Successfully..."
        );
        redirect("/all");
      } else {
        toast.error("An Error occured while updating, Try again ");
      }
    } catch (error) {
      console.log(error);
    }
